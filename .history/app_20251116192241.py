...
from scripts.data_fetcher import ChessDataFetcher
from scripts.analyzer import ChessAnalyzer
from scripts.predictor import ChessPredictor
    ...
    if fetch_button and username:
        with st.spinner("Fetching games from Chess.com..."):
            try:
                ...
                if games:
                    df = fetcher.process_and_save(username, games)
...
if st.session_state.data_loaded and st.session_state.df is not None:
    ...
    display_columns = [
        ...
        'result_label', 'user_color', 'opening', 'game_url'
    ]
    ...
    display_df['Game Link'] = display_df['Game Link'].apply(
        lambda x: f'<a href="{x}" target="_blank">View Game</a>' if pd.notna(x) else ''
    )
