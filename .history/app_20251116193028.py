...
with st.sidebar:
    ...
    col1, col2 = st.columns(2)
    with col1:
        fetch_button = st.button("🔄 Fetch Date Range", type="primary", use_container_width=True)
    with col2:
        fetch_all_button = st.button("📥 Fetch All Games", use_container_width=True)
    ...
    if fetch_all_button and username:
        with st.spinner("Downloading all games (this may take a while)..."):
            try:
                fetcher = ChessDataFetcher()
                ...
                if df is not None and len(df) > 0:
                    ...
                    st.session_state.data_loaded = True
                    st.session_state.username = username
                    st.success(f"✅ Loaded {len(df)} games from complete history!")
                else:
                    st.error("No games found.")
            except Exception as e:
                st.error(f"Error fetching all games: {e}")
...
"""
    ...
    2. **Choose your fetch method**:
       - **Fetch Date Range**: Get games from a specific time period
       - **Fetch All Games**: Download your complete game history (recommended for first use)
    3. Explore your performance metrics, trends, and predictions!
