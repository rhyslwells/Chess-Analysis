...
def render_competitor_analysis(analyzer):
    ...
    elo_df = pd.DataFrame([{"Username": u, "Elo": e if e is not None else "N/A"} for u, e in competitor_elos.items()])
    ...
    for comp_user, comp_elo in competitor_elos.items():
        if comp_elo is None:
            st.warning(f"No {time_control} rating found for {comp_user}.")
            continue
