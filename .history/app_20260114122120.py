...
def render_competitor_analysis(analyzer):
    ...
    def get_prob_for_elo(curve_df, comp_elo):
        """Return the win probability for the closest opponent Elo in the curve."""
        if curve_df.empty:
            return None
    ...
    for comp_user, comp_elo in competitor_elos.items():
        ...
        # Get win probability curves from predictor
        ...
        prob_white = get_prob_for_elo(curve_white, comp_elo)
        prob_black = get_prob_for_elo(curve_black, comp_elo)

        table_data.append({
            "Competitor": comp_user,
            "Elo": comp_elo,
            "White Win %": prob_white,
            "Black Win %": prob_black
        })
